// A simple tool capable of encrypting and decrypting a message using sr25519 keys
package main
...
import (
	...
	"github.com/gabihodoroaga/ecdh/sr25519"
...
func main() {
	...
	encFlag := flag.Bool("e", false, "Encrypt a message using a sr25519 public key")
	decFlag := flag.Bool("d", false, "Decrypt a message using a sr25519 private key")
...
func encrypt(key, msg []byte) ([]byte, error) {
	epkb, epubb, err := sr25519.GenerateKeyPair()
	if err != nil {
		return nil, err
	}

	secret, err := sr25519.SharedSecret(key, epkb)
	if err != nil {
		return nil, err
	}
...
func decrypt(key, msg []byte) ([]byte, error) {
	...
	secret, err := sr25519.SharedSecret(ephPub, key)
	if err != nil {
		return nil, err
	}
